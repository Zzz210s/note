...
```text
0-Note\
├── 00-索引.md  Root index. One global page: every project's name / status / knowledge count / entry
...
|------|------|--------|--------------------|
| **00-索引.md** | Root index · one global page | Links and routes only: one line per directory under `10-项目/` (name / status / knowledge count / entry); content lives in the owner's `20-知识\` | the `## 项目清单` table of the root `00-索引.md` |
...
- **The reverse is a hard error (A12)**: a project sitting under `40-归档/` (a folder holding `!项目说明.md`) must keep a line in the root `00-索引.md`; an archived project that vanished from the index is a file nobody can find again. Keep the line - tick its `## 计划与进度` entry and set its `## 项目清单` status to `归档`
...
- Project folder naming = time-granularity first + `!` prefix for pinned daily items; granularity encodes difficulty: months for beginner topics, quarters for foundations/deployment, years for advanced internals. The exact learning order inside a group lives in the root `00-索引.md` "learning route" section
...
- Two levels: the root `00-索引.md` lists every project (one global page); each project's `00-索引.md` lists that project's notes + recommended order + gaps to fill
...
- Vault checker (A1-A14, exit code 0 = PASS): `cd 50-资源/工具/vault-check && PYTHONIOENCODING=utf-8 python -B check_vault.py`; `--coverage` for per-index coverage, `--moc-stats` for entry counts. The two hint channels (archive-ready projects, root index not yet created) print their own `[提示]` block and never count as FAIL
...
- Index pages: `00-索引.md` - one global page at the root, one per project/container (e.g. `10-项目/!一天一道算法题/00-索引.md`)
...
- Update the owning project's `00-索引.md` after every big change, and the root `00-索引.md` whenever projects are added, renamed or archived
...
4. **Project (10-项目)** - if this becomes systematic practice, create `10-项目/2026-10-刷完Hot100/` with goal, deadline, task list; cross-link via `related: [[..]]` so project and note hook into each other, then add the project's line to the root `00-索引.md`.
5. **Archive (40-归档)** - once the project meets the four archive criteria the checker hints `可归档:<project>`; the whole folder then moves into `40-归档` (append-only) while keeping its line in the root `00-索引.md`. A single note that only ever served one project is not moved alone - it is retyped as `note` / `log` instead.

```text
idea/material -> 50-资源 (staging) -----\
                                        +-> 10-项目/<项目>/20-知识 (finished notes) -> 40-归档 (sealed) <- move when done
templates 90-模板 <-copy- write body----+                  ^
                                        \-> 00-索引.md (root + project index pages)
```
