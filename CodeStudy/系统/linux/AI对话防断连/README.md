...
**claude / opencode 等无自带滚轮的 TUI：用 `tmux-scroll-toggle` 插件**（本地插件 `~/.tmux/plugins/tmux-scroll-toggle/`）。默认 `mouse on`（滚动模式），滚轮翻历史、拖拽透传不自动复制；`Ctrl+b m` 切到「选区模式」（原生选区/复制）。

| 操作 | 方式 |
| --- | --- |
| 切换 选区/滚动 模式 | `Ctrl+b` `m` |
| 滚动模式(默认)：滚轮翻历史 | 滚轮 → copy-mode，滚到底自动退出 |
| 滚动模式：拖拽 | 透传给应用，不选区/不复制 |
