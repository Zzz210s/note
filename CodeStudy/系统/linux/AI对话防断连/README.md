...
3. **鼠标选择与复制粘贴** —— 终端里可直接用鼠标划选文本、右键/快捷键复制粘贴，不被 tmux 劫持。
...
终端体验（颜色、鼠标、按键延迟）由 `.tmux.conf` 统一调优，保证不卡顿、鼠标行为与图形终端原生一致。

## 组成

| 文件 | 作用 |
| --- | --- |
| `.tmux.conf` | tmux 终端调优：真彩色、回滚缓冲、按键延迟、鼠标不劫持、OSC52 剪贴板、崩溃恢复 |
...
# 3) 鼠标：在会话里用鼠标划选文本，应能直接用系统选区复制（tmux 不拦截）。
