...
A7 的来源页 = `L.route_sources()`(根 `00-索引.md` + 各项目 `00-索引.md`,过渡期再加旧的
`00-索引/系统.md`)。反向判定「项目是否已进路线」按来源页**全文**里指向该项目
`!项目说明.md` 的链接(同时认 `](...)` 与 `[[...]]`):只扫「计划与进度」小节会漏掉写在
表格/别处的条目,而 Task 8 生成的根索引两种写法都可能出现。零命中时只发一条汇总提示,
不逐项目刷「未进路线」(删旧 MOC 的当天否则会一次冒出十几个)。路线小节标题须是行首
二级标题(`## 计划与进度` / `## 学习路线`,后者旧文本带注解,故按前缀匹配)。
