...
pi 的 provider 密钥全部是**明文 JSON 文件**,不存在数据库/凭据管理器:

| 文件 | 作用 | 结构 |
|---|---|---|
| `~/.pi/agent/auth.json` | 各 provider 的 API Key(主存储) | 顶层键 = provider 名,每个值是 `{ "type": ..., "key": ... }` |
| `~/.pi/agent/model-hub.json` | model-hub 扩展的多 key 轮换池 | `providers[]`(含多个 key 实例)+ `aliases` |
...
### 2. 从 model-hub.json 移除(若该 provider 也在轮换池里)

```bash
node -e "const fs=require('fs'),h=process.env.HOME,p=h+'/.pi/agent/model-hub.json'; const m=JSON.parse(fs.readFileSync(p,'utf8')); m.providers=(m.providers||[]).filter(x=>x.id!=='ark-coding'); fs.writeFileSync(p, JSON.stringify(m,null,2))"
```
...
- **defaultProvider**:若被删的是默认 provider,先改 `~/.pi/agent/settings.json` 的 `defaultProvider` 指向现存 provider,否则 pi 启动报无默认 provider:
  ...
  ```
- `/model` 菜单与 model 别名:model-hub.json 的 `aliases` 若引用该 provider 的模型,一并清理
...
cp ~/.pi/agent/model-hub.json ~/.pi/agent/model-hub.json.bak.$(date +%Y%m%d)

# 2) 把 auth.json 重置为仅含空占位(而非删除文件——删除后 setup.sh 会用模板重建,行为一致)
echo '{}' > ~/.pi/agent/auth.json

# 3) 清空 model-hub 轮换池(保留空结构)
node -e "const fs=require('fs'),p=process.env.HOME+'/.pi/agent/model-hub.json'; fs.writeFileSync(p, JSON.stringify({providers:[],aliases:{}},null,2))"

# 4) settings.json 的 defaultProvider 改回可用项或 'google'(pi 内置)
```
...
| model-hub 状态文件 | `~/.pi/agent/model-hub-state.json`(429/重置时间戳)无敏感内容,可留可删 |
